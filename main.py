...
if (selected == 'Heart Disease Prediction'):
    ...
    with col2:
        sex = slt.text_input('Sex')

    with col3:
        cp = slt.text_input('Chest Pain types')

    with col1:
        trestbps = slt.text_input('Resting Blood Pressure')

    with col2:
        chol = slt.text_input('Serum Cholestoral in mg/dl')

    with col3:
        fbs = slt.text_input('Fasting Blood Sugar > 120 mg/dl')

    with col1:
        restecg = slt.text_input('Resting Electrocardiographic results')

    with col2:
        thalach = slt.text_input('Maximum Heart Rate achieved')

    with col3:
        exang = slt.text_input('Exercise Induced Angina')

    with col1:
        oldpeak = slt.text_input('ST depression induced by exercise')

    with col2:
        slope = slt.text_input('Slope of the peak exercise ST segment')

    with col3:
        ca = slt.text_input('Major vessels colored by flourosopy')

    with col1:
        thal = slt.text_input('thal: 0 = normal; 1 = fixed defect; 2 = reversable defect')
